...
import { NoticeItem, NoticeIconList } from '@shared/components/notice-icon';
...
@Component({
    ...
    <notice-icon
        ...
        [count]="count"
        [loading]="loading"
        (popoverVisibleChange)="loadData($event)"
        (select)="select($event)"
        (clear)="clear($event)"></notice-icon>
    `
})
